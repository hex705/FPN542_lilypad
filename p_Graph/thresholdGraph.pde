...
ThresholdGraph(){
  ...
  void update(){
    ...
    for (int i = 0; i < xPosArr.length; i++){
      ...
      // draw poti at xpos 
      rect(xPosArr[i]*scaleValue*threshScale[i]-potiWidth/2, i*potiHeight, potiWidth, potiHeight);
      
      // draw xpos as text
      fill(0);
      text(xPosArr[i]+inputName , xPosArr[i]*scaleValue*threshScale[i], i*potiHeight+potiHeight/2);
      }//end for
